...
检查项目中是否存在 `knowledge/history-code` 目录：
- **存在**：执行此步骤前先询问用户「是否读取 `knowledge/history-code` 进行分析」，获得确认后再继续。
- **不存在**：跳过此步骤。

读取该目录下的代码，分析时**只关注**以下两类内容：
- **系统功能**：实现了哪些功能、业务规则、数据状态流转、边界条件
- **交互逻辑**：用户操作如何触发行为、数据如何在交互中流动、各功能/页面间的联动关系

**明确忽略**（不纳入分析，也不写入产出物）：
- 技术栈、框架、依赖、构建配置
- 页面样式、CSS、布局/视觉细节

将分析结果用于补充下一步生成的系统功能文档，重点充实截图无法体现的部分：「页面详细说明」中的主要交互与业务逻辑、功能间的联动、「待实现功能」等。

---

### 第8步：生成系统功能文档

读取所有截图和内容，结合源码与历史代码分析结果（如有），生成 `SDD/snapshot/system-overview.md`。
